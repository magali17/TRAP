...
APOE
## ?? also look at AP median @ baseline? 
? NMAR b/c less likely to be missing if you were dx w/ dementia or AD? 
...
apoe.w %>%
  glm(apoe_missing ~age_last_visit,
  family = "binomial",
  data = .
  ) %>%
  summary()
...
```{r}
#on IPW for missing APOE values....
...
dem.bsl <- dem1 %>%
  ...
  group_by(study_id, pollutant) %>%
  filter(
    #exposure for intake year  
    age_end_exposure == age_intake
    ) %>%
...
### --> compare #s to when use summaryFull()? 

```{r}
# ?? why are #s slightly different than Missingness table? 
# #All, using summaryFull()
# dem.bsl %>%
#   select(
