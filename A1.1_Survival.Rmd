...
pacman::p_load(survival, tidyverse, knitr, Hmisc, EnvStats, lubridate, glmnet,
               survminer, kableExtra
...
source(file.path("A1.0.2_Var&Fns.R"))
...
dem1 <- dem0.1 %>% 
  ...
  mutate(
    ...
    exp_avg10_yr20yrlag_original = exp_avg10_yr20yrlag,
...
no2 <- dem1 %>% 
  ...
  rename(
    ...
    no2_10yr20yrlag_original = exp_avg10_yr20yrlag_original,
...
pm25 <- dem1 %>% 
  ...
  rename(
    ...
    pm25_10yr20yrlag_original = exp_avg10_yr20yrlag_original,
...
### --> use same variables Rachel uses & check models
...
selected_cov <- selected_cov %>%
  #change back to orinal name
  str_replace(pattern = "bmi_cat", replacement = "bmi4" )

#paste0(selected_cov, collapse = ", ")
...
Summary of Lasso-selected stabilized weights used in coxph()
...
### --> age at entry vs (?)proportion of individuals who develop dementia. ? Among ppl followed the same length of time (otherwise those who enter later have less opportunity to develop dementia)? 

```{r}
fixed.covariates %>%
  mutate(
    age_intake_cat = cut(age_intake, breaks = 10)
  ) %>%
  group_by(age_intake_cat) %>%
  dplyr::summarize(
    prop_who_devp_dem = mean(anydementia==1)
  )

#names(fixed.covariates)
...
#     pollutant == "no2") %>%
#   unique() %>%
#   t1.fn(data=., column.name = "All")
# 
# kable(t1.all, caption = "Cohort Baseline Characteristics") %>%
#   add_indent(c(6:7, 9:14,16:19,22:24, 27:30, 32:35))
#   
# 
...
#     pollutant == "no2") %>%
#   filter(!is.na(apoe)) %>%
#   t1.fn(data = ., column.name = "Available")
# 
...
#     pollutant == "no2") %>%
#   filter(is.na(apoe)) %>%
#   t1.fn(data = ., column.name = "Missing") %>%
#   rownames_to_column() %>%
#   #get rid of NANs since trying to divide by 0 available readings
#   mutate(Missing = str_replace(string = Missing, pattern = "0 \\(NaN%\\)", replacement =  "NA")) %>%
#   column_to_rownames()
# 
# 
# #t1.m2 <- cbind(t1.apoe.yes, t1.apoe.no, t1.apoe.unavail)
# t1.m2 <- cbind(t1.apoe.avail, t1.apoe.unavail)
# 
# kable(t1.m2, caption = "Cohort characteristics by whether subjects have APOE readings available") %>%
#   add_indent(c(6:7, 9:14,16:19,22:24, 27:30, 32:35)) %>%
#   add_header_above(c("", "APOE" = 2))
# 
# #kable_styling()
...
t1.apoe.dem <- dem1 %>%
  ...
  filter(!is.na(apoe),
         anydementia == 1) %>%
  t1.fn(data = ., column.name = "apoe avail & dementia")
...
t1.apoe.nodem <- dem1 %>%
  ...
  filter(!is.na(apoe),
         anydementia == 0) %>%
  t1.fn(data = ., column.name = "apoe avail & no dementia")
...
t1.noapoe.dem <- dem1 %>%
  ...
  filter(is.na(apoe),
         anydementia == 1) %>%
  t1.fn(data = ., column.name = "no apoe avail & dementia") %>%
  rownames_to_column() %>%
  #get rid of NANs since trying to divide by 0 available readings
  mutate("no apoe avail & dementia" = str_replace(get("no apoe avail & dementia"), pattern = "0 \\(NaN%\\)", replacement =  "NA")) %>%
  column_to_rownames()

t1.noapoe.nodem <- dem1 %>%
  filter(
    #1st year of enrollment (baseline)
    enrollment_yr == 1,
    pollutant == "no2") %>%
  filter(is.na(apoe),
         anydementia == 0) %>%
  t1.fn(data = ., column.name = "no apoe avail & no dementia")%>%
  rownames_to_column() %>%
  #get rid of NANs since trying to divide by 0 available readings
  mutate("no apoe avail & no dementia" = str_replace(get("no apoe avail & no dementia"), pattern = "0 \\(NaN%\\)", replacement =  "NA")) %>%
  column_to_rownames()

#table1
cbind(t1.apoe.dem, t1.apoe.nodem, t1.noapoe.dem, t1.noapoe.nodem) %>%
  kable(caption = "Cohort characteristics by Dementia Incidence and APOE availability", 
        col.names = c("", "", "", ""), 
        align = "c", 
        ) %>%
  add_indent(c(7:12,14:17,20:22, 25:28, 30:33)) %>%
  add_header_above(c("", "Dementia" = 1, "No Dementia" = 1,  "Dementia" = 1, "No Dementia" = 1)) %>%
...
t1.apoe.ad <- dem1 %>%
  ...
  filter(!is.na(apoe),
         ad_nincds == 1) %>%
  t1.fn(data = ., column.name = "apoe avail & AD")
...
t1.apoe.noad <- dem1 %>%
  ...
  filter(!is.na(apoe),
         ad_nincds == 0) %>%
  t1.fn(data = ., column.name = "apoe avail & no AD")
...
t1.noapoe.ad <- dem1 %>%
  ...
  filter(is.na(apoe),
         ad_nincds == 1) %>%
  t1.fn(data = ., column.name = "no apoe avail & AD") %>%
  rownames_to_column() %>%
  #get rid of NANs since trying to divide by 0 available readings
  mutate("no apoe avail & AD" = str_replace(get("no apoe avail & AD"), pattern = "0 \\(NaN%\\)", replacement =  "NA")) %>%
  column_to_rownames()
...
t1.noapoe.noad <- dem1 %>%
  ...
  filter(is.na(apoe),
         ad_nincds == 0) %>%
  t1.fn(data = ., column.name = "no apoe avail & no AD")%>%
  rownames_to_column() %>%
  #get rid of NANs since trying to divide by 0 available readings
  mutate("no apoe avail & no AD" = str_replace(get("no apoe avail & no AD"), pattern = "0 \\(NaN%\\)", replacement =  "NA")) %>%
  column_to_rownames()

#table1
cbind(t1.apoe.ad, t1.apoe.noad, t1.noapoe.ad, t1.noapoe.noad) %>%
  kable(caption = "Cohort characteristics by Alzheimer's Disease incidence and APOE availability", 
        col.names = c("", "", "", ""), 
        align = "c", 
        ) %>%
  add_indent(c(7:12,14:17,20:22, 25:28, 30:33)) %>%
  add_header_above(c("", "AD" = 1, "No AD" = 1,  "AD" = 1, "No AD" = 1)) %>%
  add_header_above(c("", "APOE Available" = 2, "APOE Not Available" = 2))
...
avg_no2_by_avg_prds <- dem1 %>%
  select(exposure_year, exp_avg01_yr:exp_avg20_yr) %>%
  gather(key = "Avg_Prd", value = "NO2_ppb" , -exposure_year) %>%
  mutate(Avg_Prd = as.factor(substr(Avg_Prd, 8, nchar(Avg_Prd)))) %>%
...
dem1 %>%
  ggplot(aes(x=exposure_year, y = exp_avg01_yr)) +
  ...
  geom_line(data=avg_no2_by_avg_prds, aes(y=mean_no2_ppb, col=Avg_Prd)) + 
    labs(x = "Year",
         y = "NO2 Exposure (ppb)",
         col = "Averaging Period",
         title = "NO2 exposure over time in the cohort", 
         subtitle = "black points: individual exposure predictions;\ncolored points: average exposure predictions for that year and averaging period"
           )

# ggsave(filename = file.path("A1_Images", "NO2 over time by avg prd2.png"), 
#        width = 8)    

#formula = y ~ splines::bs(x, 3)
```

#### --> ? parallel plot for e.g., 2 ppl who never moved (show how their estimates decreased over time & how diff avg periods affect these estimates)
...
no2.all.yrs.py <- dem1 %>% expo.distrib.fn(dt = ., years.description = "All", var. = "exp_avg10_yr")

no2.yr1.py <- dem1 %>%
  filter(exposure_year <= 2000) %>%
expo.distrib.fn(dt = ., years.description = "1994-2000", var. = "exp_avg10_yr")

no2.yr2.py <- dem1 %>%
  filter(exposure_year >= 2001, 
         exposure_year <= 2009) %>%
expo.distrib.fn(dt = ., years.description = "2001-2009", var. = "exp_avg10_yr")

no2.yr3.py <- dem1 %>%
  filter(exposure_year >= 2010) %>%
  expo.distrib.fn(dt = ., years.description = "2010-2018", var. = "exp_avg10_yr")
...
no2.by.yrs.py %>% 
  ...
  select(-pollutant) %>%
  kable(., digits = 0,
        caption = "10-yr average NO2 (ppb) exposure predictions over time for everyone in Cohort (N = person-years)")
...
no2.by.yrs.py %>% 
  ...
  select(-pollutant) %>%
  kable(., digits = 0,
        caption = "10-yr average PM2.5 (ug/m3) exposure predictions over time for everyone in Cohort (N = person-years)")
...
dem_10yr_hrs <- dem_10yr$hrs %>%
  mutate(
    ...
    Description = c("Reduced",
           "Primary", 
           "Extended", 
           "Extended, includes ACT Cohort", 
           "Extended & Mediation", 
           "Extended & Mediation, includes CASI Score", 
           "Interaction, no APOE", 
           "Interaction, APOE", 
           "PM2.5 Adjusted")
    )
...
dem_10yr_act <- dem_10yr_act$hrs %>%
  mutate(Model = "2",
         Description = "Using ACT onset age"
         )
...
### --> [emailed Roy] Only include individuals for whom we have complete air pollution predictions and address history during the exposure period of interest
