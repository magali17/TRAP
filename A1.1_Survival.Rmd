...
### --> ? create dem.bsl in T1 fn instead so can calculate person-years using nrow(dem1)?
...
```{r}
# # 1 yr correlations
# (no2_pm25_1yr_r2 <- with(dem.w, 
#                          cor(no2_1yr, pm25_1yr, 
#     method = "pearson", 
#     use = "complete.obs")^2))
...
### --> distribution of degree (collapse categories? )
...
table(fixed.covariates$degree)
table(fixed.covariates$education)


# how is degree categorized???
table(degree = dem.bsl$degree, edu = dem.bsl$education)
...
### --> add person-years to T1? straighforward for whole cohort (sum(fu_yrs)); but need nrow() for dataset used in strata? 
...
```{r}
#dataset with variables of interest for use in model function.  
model.vars <- dem.w %>%
  select(
    #m1 & sensitivity
    no2_1yr:no2_20yr ,
  ...
  #make factors
    ...
    #make 10 ppb units
    no2_10ppb_1yr = no2_1yr/no2.units,
    no2_10ppb_5yr = no2_5yr/no2.units,
    no2_10ppb_10yr = no2_10yr/no2.units,
    no2_10ppb_20yr = no2_20yr/no2.units,
    no2_10ppb_10yr10yrlag = no2_10yr10yrlag/no2.units,
    no2_10ppb_10yr20yrlag = no2_10yr20yrlag/no2.units,
    
    #make 5 mg/m3
    pm25_5ugm3_1yr = pm25_1yr/pm25.units,
    pm25_5ugm3_5yr = pm25_5yr/pm25.units,
    pm25_5ugm3_10yr = pm25_10yr/pm25.units,
    pm25_5ugm3_20yr = pm25_20yr/pm25.units,
    pm25_5ugm3_10yr10yrlag = pm25_10yr10yrlag/pm25.units,
    pm25_5ugm3_10yr20yrlag = pm25_10yr20yrlag/pm25.units,
...
See high correlatin (R^2) between NO2 and PM2.5
...
R2 for highly corelated variables (NO2 and PM2.5)
...
10 ppb NO2, 10-year avg 
## --> FIX models.fn(): M6 hr for apoe carriers is wrong. should be exp(no2 + no2:apoe); manually calculate CI & ?pval; create new variable to get HR out?
## --> ? add to fn: plots of HR w/ CIs vs model # (Tzivian 2016)
...
#### --> double check that these #s are correct

```{r}
#need new Surv() object
model.vars %>%
  #prep the dataset for use in the function, set correct no2 estimate you want to use in the model
  rename(
    no2 = no2_10ppb_10yr,
...
Using past 1-year average exposure for NO2 and PM2.5 
...
Using past 5-year average exposure for NO2 and PM2.5 
...
Using past 20-year average exposure for NO2 and PM2.5 
...
model.vars %>%
  ...
  rename(
    no2 = no2_10ppb_20yr,
    pm25 = pm25_5ugm3_20yr
    ) %>%
  models.fn(model.vars.data = .) 

```

Using past 10-year w/ 10 yr lag average exposure for NO2 and PM2.5 

#### --> error. infinite values created. too few ppl? 

```{r}
# model.vars %>%
#   #prep the dataset for use in the function, set correct no2 estimate you want to use in the model
#   rename(
#     no2 = no2_10ppb_10yr10yrlag ,
#     pm25 = pm25_5ugm3_10yr10yrlag
#     ) %>%
#   models.fn(model.vars.data = .) 

```

Using past 10-year w/ 20 yr lag average exposure for NO2 and PM2.5 
...
#   #prep the dataset for use in the function, set correct no2 estimate you want to use in the model
#   rename(
#     no2 = no2_10ppb_10yr20yrlag ,
#     pm25 = pm25_5ugm3_10yr20yrlag
#     ) %>%
#   models.fn(model.vars.data = .) 
