...
dem0.1 <- dem0.0 %>% 
  ...
  mutate(
    ...
    #original income catgory 1 only  had 7 individuals (too small)
    ...
    # --> ?replace degree=9 w/ NA? no definition for degree=9 in data dictionary
    degree = ifelse(degree != 9, degree, NA)
...
(no2_10yr_models <- models.fn() #[[1]]
 ) 

# t <- models.fn()
# t <- t$table_of_no2_HRs  

```

## --> fix birth_cohort30 has NAs
-birth_cohort30 is associated with: only 1 person w/ 11 fu yrs, never gets dementia 

```{r}
#only 1 person (11 entries) in ref category for birth_cohort1930
model.vars %>% #mydata
  filter(male==0,
         race_white==0,
         income == 1,
...
(no2_1yr_models <- models.fn(no2.var = "no2_1yr", 
          pm25.var = "pm25_1yr"
          ) #[[1]]
)
...
```{r}
(no2_5yr_models <- models.fn(no2.var = "no2_5yr", 
          pm25.var = "pm25_5yr"
          ) #[[1]]
)
...
```{r}
(no2_20yr_models <- models.fn(no2.var = "no2_20yr", 
          pm25.var = "pm25_20yr"
          ) #[[1]]
)
...
```{r}
(no2_10yr10yrlag_models <- models.fn(no2.var = "no2_10yr10yrlag", 
          pm25.var = "pm25_10yr10yrlag"
          ) #[[1]]
)
...
(no2_10yr20yrlag_models <-models.fn(no2.var = "no2_10yr20yrlag", 
          pm25.var = "pm25_10yr20yrlag"
          ) #[[1]]
)
...
(ad_models <- models.fn(surv.event = "ad_now",
            outcome.text = "AD"
          ) #[[1]] 
)
...
#### --> ? add other exposure time periods
