...
### --> hsit of # of missing apoe by enrollment date/yr
...
#### --> error w/ missing_apoe variable?

```{r}
t <- dem.w %>%
  filter(enrollment_yr==1) %>%
  mutate(missing_apoe = (is.na(apoe))) #%>%
  
ggplot(aes(x=exposure_year, y = missing_apoe)) + 
  geom_bar(aes(stat_count()))
...
summary(dem.w$last_act_fu_yr)
table(dem.w$last_act_fu_yr)
table(dem.w$last_act_fu_yr)
...
### --> add table/list of HRs to plots? add 2nd axis? 
