...
dem0.0 <- haven::read_sas(file.path("Data", "Aim 1", "issue_001_200225.sas7bdat"), NULL)
...
dem0.1 <- dem0.0 %>% 
  ...
  select(
    #final_nindx has issues if this is still here. not sure why.
    -nindx,
    # study_id,
    # birthdt,
    # intakedt,
...
## Coverage Proportion
Plot Coverage variable against participant enrollment year. See that coverage is more likely to be 1 at the begining of participant enrollment. This is because the coverage variables takes the imputed variables prior to enrollment into account. 

-Important to look at this variable because we were originally excluding person-years based on this variable
...
### --> does this go down after ~ 2006 b/c of large, initial cohort has been followed > 10 yrs and no longer imputing coverage? 
...
### --> ? Why is imputed coverage > 0 after 10 years for the 10 year imputed coverage variable? Is this for people for whom we do not know their address at baseline? 

```{r}
#  
dem.w %>%
  ggplot(aes(x=enrollment_yr, y = imputed_coverage10_yr)) + 
  geom_point(alpha = 0.05) + 
  geom_smooth() + 
  labs(
    x = "Participant Enrollment Year in Study",
...
### --> 1) why is imputed coverage > 0 after 10 years? Is this for people w/o baseline addresses?
### --> 2) why do some person-years have imputed coverage = 1? 100% of the values are imputed from what?  
### --> 3) 3) are imputed coverage values decreasing over time b/c we have residential hx for these people through the Group Health/KP system even if they were not in the study?

```{r}
dem.w %>%
  ggplot(aes(x=exposure_year, y = imputed_coverage10_yr)) + 
  geom_point(alpha = 0.05) + 
  geom_smooth() + 
  labs(
    x = "Year",
...
summary(dem.w$imputed_coverage10_yr)

# ? why are some imputed values = 1? No address at baseline?
table(dem.w$imputed_coverage10_yr==1)
...
```{r}
dem.w %>%
  ggplot(aes(x=exposure_year, y = exact_coverage01_yr)) + 
  geom_point(alpha = 0.05) + 
  geom_smooth() + 
  labs(
    x = "Year",
...
dem.w %>%
  ggplot(aes(x=enrollment_yr, y = exact_coverage01_yr)) + 
  geom_point(alpha = 0.05) + 
  geom_smooth() + 
  labs(
    x = "Participant Enrollment Year in Study",
...
### --> ? why do we still have so much missingness in this variable? Amanda? 

```{r}

dem.w %>%
  ggplot(aes(x=is.na(exact_coverage01_yr), y = is.na(tr_med_inc_hshld))) + 
  geom_point(alpha = 0.05) + 
  geom_smooth() + 
  labs(
    # x = "Participant Enrollment Year in Study",
...
# table(exact_na = is.na(dem.w$exact_coverage01_yr), inc_na = is.na(dem.w$tr_med_inc_hshld))
...
Plots of APOE availability. See slightly proportinally more missing APOE readings later in time. This may be because they have not yet had their blood work processed.
...
# Create fixed covariates dataset 
