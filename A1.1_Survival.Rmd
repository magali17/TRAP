...
dem_10yr_hrs <- dem_10yr$hrs %>%
  mutate(
    ...
    Description = c("Reduced",
           "Primary", 
           "Extended", 
           "Extended, with ACT Cohort", 
           "Extended & Mediation", 
           "Extended & Mediation, with CASI Score", 
           "Interaction, no APOE", 
           "Interaction, APOE", 
           "PM2.5 Adjusted")
    )
...
### --> plot reduced/extended: 10 yr M1-6, 
### --> plot other sensitivity: 10 yr M2
## ? use hr.plot()
...
dem_all_hrs %>%
  filter(
    Model != "2" | (Model == "2" & Description == "Primary")
    ) %>%
  ggplot(aes(x=Model, y=hr, col=factor(Description), size = factor(exposure))) + 
