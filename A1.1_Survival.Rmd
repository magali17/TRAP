...
pacman::p_load(survival, haven, tidyverse, knitr, Hmisc, EnvStats, lubridate, glmnet)  #Himsc: describe(); EnvStats: summaryFull()  
...
#### --> Make T2: exposure table for a range of calendar years and age group rather than just baselineo. ? 3x3 table w/ margin estimates & middle estimate. Shows heterogeneity of exposure 

#### ---> also,	Summaries over time, ppl r included multiple times 
...
#### --> 
#characteristics for those included in M2 

```

#### --> add T1 for those included in M2 (have APOE)
...
mynumeric = as.numeric(c(TRUE, TRUE, FALSE, FALSE, FALSE, TRUE, TRUE, FALSE, FALSE, FALSE, FALSE, FALSE, TRUE, FALSE, TRUE, FALSE))

#replace  missing values w/ mean (if numeric) or mode (if factor)
for (i in seq_along(with.nas)) {
  #vector w/ missing values 
  myvar <- with.nas[i] #as.character()
...
#use gender probability in numerator to stabilize weights (propensity scores)
male.prop <- mean(lasso.m.vars$male)

lasso.m.vars <- lasso.m.vars %>%
  mutate(
    model_wt = ifelse(male==1, male.prop/apoe_available_prob, (1-male.prop)/apoe_available_prob) 
  )
...
#table of all HRs for all times & models
kable(dem.wts.results$HRs, caption = "Dementia HRs")
...
hr.comp %>%
  filter(unweighted_hr <= quantile(unweighted_hr, 0.98) & 
           weighted_hr <= quantile(weighted_hr, 0.98)
           ) %>%
...
3. Model "1b" w/ APOE stratification
