...
#table1
t1_dem <- cbind(t1.apoe.dem, t1.apoe.nodem, t1.noapoe.dem, t1.noapoe.nodem) %>%
...
#table1
t1_ad <- cbind(t1.apoe.ad, t1.apoe.noad, t1.noapoe.ad, t1.noapoe.noad) %>%
...
ANOVA - what is responsible for the variability in exposure estimates?
...
dem1 %>%
  ggplot(aes(x=exposure_year)) +
  geom_point(alpha=0.1, aes(y = exp_avg01_yr, fill = "")) +
...
no2_all_yrs_py <- dem1 %>%
  mutate(`Year Range` = "all years") %>%
  group_by(`Year Range`, pollutant) %>%
  drop_na(exp_avg10_yr) %>%
  distribution.table(var.string = "exp_avg10_yr")

no2_by_yr_range <- dem1 %>%
  mutate(`Year Range` = cut_interval(x = exposure_year, n = 4)) %>%
  # group_by(yr_range) %>%
  # dplyr::summarize(
  #   min = min(exposure_year),
  #   max = max(exposure_year)
  # )
  group_by(pollutant, `Year Range`) %>%
    drop_na(exp_avg10_yr) %>%
    distribution.table(var.string = "exp_avg10_yr")

no2.by.yrs.py <- rbind(no2_all_yrs_py, no2_by_yr_range)
