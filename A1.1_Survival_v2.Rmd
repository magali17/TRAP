...
-both are still "away" from roadways
```{r}
mean(c(rep(1500, 8), rep(10, 2)))  # 1202

# log-transform distnace, then exponentiate
mean(c(rep(log(1500), 8), rep(log(10), 2))) %>% exp()  # 550 
...
define "near road" as:   
a) < 150 m from A1 or < 50 m A2 for longest-lived address *OR* use TWA of all addresses after max distance is set to ~500 m
b) < 400 m from A1 or < 200 m A2... 
...
using the longest-lived address may add some exposure misclassification to this analysis because it assumes this a person lived at the same location or a comparable one the entire time.

### --> ? only using longest-lived address?
...
#### log(distance to A1 or A2)

natural log of distnace has been linearly related to martality and morbidity outcomes.(Chen 2017  living near; Wilker 2014 & 2015; 24: chen 2012)

### --> truncate max distance to 1500 m before taking average
...
#### Year-specific categories (calendar year) 
...
dem_cal_time_hrs <-
  ...
  mutate(
    ...
    year_cohort = factor(round(exposure_year/round_year)*round_year)
  ) %>%
  ...
  mutate(
    Exposure = "10yr",
    Model = "2",
    Description = "Calendar Year Bin Instead of Birth Cohort") 
...
ad_cal_time_hrs <- mydata %>%
  ...
  mutate(
    Exposure = "10yr",
    Model = "2",
    Description = paste0("Categorical Calendar Yr (", round_year, " yr) Instead of Birth Cohort"))
...
dem_spline_cal_time_hrs <- mydata %>%
  ...
  mutate(
    Exposure = "10yr",
    Model = "2",
    Description = "Spline Calendar Year Instead of Birth Cohort") 
...
Plots checking to see if the proportional hazards assumptions appear to be true.

**Note**
-these do not include CIs
...
dem_stratification_hrs <- mydata %>%
  coxph(s.dem ~ no2 + strata(apoe) + strata(male) + strata(race_white) + strata(income) + strata(edu) + strata(birth_cohort), 
        data=., 
        cluster = study_id,
        weights = model_wt) %>% 
  summary() %>%
  ...
  mutate(
    Exposure = "10yr",
    Model = "2",
    Description = "Full Confounder Stratification") 
...
ad_stratification_hrs <- mydata %>%
  coxph(s.ad ~ no2 + strata(apoe) + strata(male) + strata(race_white) + strata(income) + strata(edu) + strata(birth_cohort), 
        data=., 
        cluster = study_id,
        weights = model_wt) %>% 
  summary() %>%
