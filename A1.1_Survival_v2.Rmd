...
dem0.1 <- dem0.0 %>% 
  ...
  select(
  study_id,
  birthdt, birth_cohort_5yr, birth_cohort, cohort, intakedt, onsetdate, last_visit,
...
```{r}
# common variables 
m2_all_vars <- c("apoe", "male", "race_white", "income_cat", "degree", "birth_cohort", 
                 #"birth_cohort_5yr", 
                 "no2_10yr") 
...
## # not stratifying table by outcome (Marco's suggestion) 
# # dementia 
# t1_dem <- dem.w %>%
#   # only keep individuals included in Model 2
#   drop_na(m2_all_vars) %>%
...
```{r}
 # 5 yr adjustment (similar to birth cohort)
round_year <- 5
...
dem_cal_time_hrs1 <- mydata %>%
  ...
  mutate(
    Exposure = "10yr",
    Model = "2",
    Description = paste0(round_year, " yr Calendar Time Instead of Birth Cohort"))
...
ad_cal_time_hrs2 <- mydata %>%
  mutate(
    # using floor instead of round() to match Rachel's 2 yr categories, which start in 1994 (e.g., 1994-1995)
    # rachel binned 2016-2018
    exposure_year = ifelse(exposure_year == 2018, 2017, exposure_year),
    year_cohort = factor(floor(exposure_year/round_year)*round_year),
  ) %>%
                #pm25
...
## Calendar time as time axis, adjusted for age?
...
dem_cal_time_axis_hrs <-
  ...
  mutate(
    ...
    birth_cohort_5yr = factor(birth_cohort_5yr),
  ...
  coxph(s.dem_calendar ~ no2 + strata(apoe) + male + race_white + income + edu +  
        birth_cohort_5yr #what primary model is using
          #year_cohort
        , 
        data=., 
        cluster = study_id,
  ...
  mutate(
    Exposure = "10yr",
    Model = "2",
    Description = "Calendar Time Axis")  #adjusted for [5 yr] Birth Cohort 
...
ad_cal_time_axis_hrs <-
  ...
  mutate(
    Exposure = "10yr",
    Model = "2",
    Description = "Calendar Time Axis") 
...
dem_near_road_cat_proximity_hrs <- mydata %>%
  coxph(s.dem ~  near_rd_cat +  
          strata(apoe) + male + race_white + income + edu + birth_cohort,
        data=., 
        cluster = study_id,
        weights = model_wt) %>% 
  summary() 
...
dem_time_near_road <- mydata %>%
  #drop_na(no2) %>%
  #convert from a 10 yr proportion to 1 yr estimates
  ...
  summary() %>%
    ...
    mutate(
      Exposure = "10yr",
      Model = "2",
      Description = paste0("Time near road")
    )
...
dem_all_hrs <- rbind(
  ...
  mutate(
    ...
    grepl("Calendar|Onset", Description) ~ "Time Variants",
    #TRUE ~ "Other",
...
dem_all_hrs %>% 
  filter(!grepl("Exposure|road|Interaction", Description)) %>%
  mutate(
    # Type =case_when(
    # grepl("Primary", Description) ~ "Primary",
    # #!grepl("Primary", Description) ~ "Sensitivity",
