...
locations_act <- cov_act1 %>%
  ...
  select(site_id = native_id,  # USED TO BE: location_id
         latitude:lambert_y, 
         site_location)
...
grid_covars1 <- grid_covars0 %>%
  ...
  rename(site_id = native_id)  # USED TO BE: location_id
...
grid_covars1 <- grid_covars1 %>%
  #only keep covariates
  select(-c(location_id, latitude:lambert_y))   # USED TO BE: native_id
