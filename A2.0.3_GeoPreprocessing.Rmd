...
# grid
grid_covars0 <- read.csv(file.path("Data", "Aim 2", "Geocovariates", "grid_covars.csv" ))  

```


```{r}
# # SIDE PROJECT: CREATE NEW GRID W/ EQUAL SPACING
# min_lat <- min(grid_covars0$latitude)
# max_lat <- max(grid_covars0$latitude)
# min_long <- min(grid_covars0$longitude)
# max_long <- max(grid_covars0$longitude)
# 
# 
# grid2 <-
...
```{r}   
######################## Only keep columns if in mm & ACT datasets ######################## 
...
Map of Census tracts with monitoring and the final monitoring area resulting from fillin in holes/gaps on the map 
...
kable(caption = "ACT cohort locations included in the monitoring and study areas relative to all U.S. locations on record", 
