...
act_in_study0 <- read.csv(file.path("..", "GIS", "Shapefiles", "ACT", "ACT_selected", "act_in_study_area.csv")) %>%
  ...
  mutate(site_location = relevel(site_location, ref = "monitoring"))  
...
data.frame(
  Locations = c("Entire Cohort", 
                ...
                #"Surrounding monitoring area"
                ),
  N = c(nrow(cov_act0), 
        NA,
        nrow(act_in_study0),
        length(act_in_study0$site_location[act_in_study0$site_location == "monitoring"])
        #length(act_in_study0$site_location[act_in_study0$site_location == "study"])
        )
) %>%
  mutate(Proportion_of_total_loc = N/nrow(cov_act0)
  ) %>%
...
## --> ? delete? using exponentiated log_scale dataset. has the additional adv that min proximity will be 10 m (similar to log-transformed values)
