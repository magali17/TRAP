...
#read in data
mm_full0 <- readRDS(file.path("Data", "Aim 2", "Mobile Monitoring", "mm_raw_2020-03-18.rda"))
...
mm_full %>%
  ...
  mutate(
    instrument_prop = round(instrument_n/variable_n, 2)
  ) %>%
...
Drop PTRAK readings from instruments PMPT_2 and PMPT_4 since these were barely used and never on their own.

### --> ?? drop PMPT_1 since it was used on its own but for very little time & never on its onw? does lab have collocaitons for PMPMT_1 w/ 93 or 94?

```{r}
mm_full <- mm_full %>%
  filter(!instrument_id %in% c("PMPT_2", "PMPT_4"))
...
mm_full %>%
  ...
  labs(
    title = "Distribution of raw aethalometer readings"
  )
...
# #bc_var <- "ona_bc"
# 
# negative_bc_no <- sum(mm_full$value[mm_full$variable == bc_var] <= 0) 
...
# page_no <- ceiling(length(unique(high_ptrak$runname))/plots_per_page)
# 
# for (i in seq(page_no)) {
#   #i=1  
#   myplot <- ptrak_day_plots +
#     ggforce::facet_wrap_paginate(~runname, #+ site_id, 
#                              nrow = 3, ncol = 4, 
#                              page = i, 
#                              scales = "free")  
...
# page_no <- ceiling(nrow(high_ptrak)/plots_per_page)
# 
# for (i in seq(page_no)) {
#   #i=2  
#   myplot <- ptrak_site_plots +
#     ggforce::facet_wrap_paginate(~runname + site_id, 
#                              nrow = 3, ncol = 4, 
#                              page = i, 
#                              scales = "free")  
