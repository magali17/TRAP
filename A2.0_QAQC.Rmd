...
### --> issue: rolling avg for PTRAKs takes VERY long time to run
...
for(i in 1:nrow(unique_times) ) {
  ...
  df <- mm_full %>% 
    ...
    #as.zoo() %>%
    mutate(
      rolling_value = roll_mean(x = value, n = 60, align = "left", fill = NA)
...
############
# # DOESNT group properly  
# library(RcppRoll) #supposed to be faster, but still slow
# 
# test <- mm_full %>%
#   slice(1:5000) %>%
...
##########
# rolling_ptrak_copy <- rolling_ptrak

test <- rolling_ptrak %>%
  #select(-value) %>%
  rename(value = rolling_mean) %>%
  mutate(variable = "ptrak_60s_rolling")

 mm_full_rolling <- rbind(mm_full, test)
