...
```{r}
# ---> TO DO:
# --> ask Tim/Edmund if missing any imp tables/figures
...
#### --> update file later: there are 310 site_ids and in mm_full. There should be 309. We stopped sampling MS0398 and switched to MS0601 which is fairly close
...
ufp.w %>%
  ...
  labs(title = paste0("Low PTRAK concentrations (< ", low.ptrak.conc, " ufp/cm3) relative to \ncomparable NanoScan (20-421 nm) concentrations"),
       ...
       x = "NanoScan (20-421 nm pt/cm3)",
       y = "PTRAK (20-1000 nm pt/cm3)"
       )
...
ufp.w %>%
  ...
  mutate(variable = recode(variable, 
                      ...
                      "scan_20_421_pt_cm3" = "NanoScan_20_421_pt")
         ) %>%
  ggplot(aes(x=time, y = value, col = variable)) + 
...
paste0("number of PTRAK seconds dropped: ", num.ptrak.seconds.dropped)
...
bc.w %>%
  ...
  geom_abline(intercept = 0, slope = 1) + 
  geom_smooth() + 
  labs(title = "UV (375 nm) concentrations before and after ONA correction",
      x = "Aethelometer (ng/m3)",
      y = "ONA-corrected (ng/m3)"
      )
...
Distribution of BC readings (log-normal?)
...
### --> reduce df size to make this plot load faster

```{r}
#instrument used each day
mm_full %>%
  #variable %in% c("ptrak_pt_cm3", "scan_20_421_pt_cm3")
  filter(variable == "ptrak_pt_cm3" | variable == "scan_20_421_pt_cm3") %>%
  unique() %>%
...
mm_full %>%  
    ...
    filter(instrument_id %in% c("PMSCAN_5", "PMSCAN_3"),
           value > quantile(value, ns_trim, na.rm = T),
           value < quantile(value, (1-ns_trim), na.rm = T)
           ) %>%
    spread(instrument_id, value) %>%
...
filter(instrument_id %in% c("PMSCAN_5", "PMPT_94"),
  ...
  colo.plot(x.variable = "scan_20_421_pt_cm3", x.label = "NanoScan 5 (20-421 nm pt/cm3)",
