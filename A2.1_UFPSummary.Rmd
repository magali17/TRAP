...
mm %>%
  filter(
    ...
    variable != "ufp_scan_20_421_nm_ct_cm3"
         ) %>%
...
mm.wide %>%
  ...
  filter(ufp_scan_20_421_nm_ct_cm3 <= quantile(ufp_scan_20_421_nm_ct_cm3, plot_quantile, na.rm = T),
         ufp_pt_noscreen_ct_cm3 <= quantile(ufp_pt_noscreen_ct_cm3, plot_quantile, na.rm = T)
         ) %>%
  colo.plot.wide.data(data.wide = ., x.variable = "ufp_scan_20_421_nm_ct_cm3",
                      y.variable = "ufp_pt_noscreen_ct_cm3", mytitle = paste0("excluding top ", (1-plot_quantile)*100, "% of data"))
...
mm.geo %>%
  filter(ufp_pt_noscreen_ct_cm3 <= quantile(ufp_pt_noscreen_ct_cm3, geo.quantile, na.rm = T)) %>%
...
mm %>%
  filter(variable == "ufp_pt_noscreen_ct_cm3") %>%
lm(value ~ factor(hour) + factor(day, ordered = F) + factor(month, ordered = F) + site_id, data=.) %>%
  summary()
...
```{r}
mm.wide %>%
  select(disc = ufp_disc_ct_cm3, 
         pt = ufp_pt_noscreen_ct_cm3, 
         pt_screen = ufp_scan_10_421_nm_ct_cm3, 
         ns = ufp_scan_20_421_nm_ct_cm3, 
         bc = bc_ir880nm_ng_m3, 
         neph = neph_bscat_per_m) %>%
  cor(method = "pearson",
      use ="complete.obs") %>%
