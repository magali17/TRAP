...
```{r}
# to do:
# --> VCA package not found? # anovaVCA() 

# --> estimate annual avg using 7 dys/wk
...
* trimmed 5% top/bottom observations per site
* plotted UFP concentrations at various times (e.g., hour, TOD, TOW, season)
* calculated stop sample sizes for different times
* calculated annual averages using various methods and weights
	+ site specific
...
* compared annual averages
	+ plots
	+ compared LUR predictions: desired estimates weighted by season, TOW2 & TOD5 vs other methods, both on the native and log-scale
* investigated sites with high inter-method variability and high regression residuals
* ran ANOVAs to investigate the fraction of UFP variability explained by various effects:
	+ method
	+ site
	+ hour/TOD
	+ TOW
	+ season 
* compared annual average estimates for different sensitivity analyses
...
#load data
mm_stops <- readRDS(file.path("Data", "Aim 2", "Mobile Monitoring", "mm_stops_2020-03-08.rda"))
...
#no. months sampled
###                   --> udpate 
months.sampled <- month.abb[c(1, 3:12)]
...
### --> make plot of UFP over hour for diff sites  
### --> see if there's an interaction btwn TOD & site location (e.g., a1 proximity)
...
rbind(
  ...
  add_row(time = "Time of Day", .before = 10) %>%
...
### --> change tables/plots "yhat" --> "regression"/"reg"; "mean" --> ?? "site-specific"
...
annual <- ufp %>% 
  ...
  # --> ADD LATER WHEN HAVE MORE WINTER READINGS?
...
### --> clean up tables for final presentation. what % of total variability is between sites? within sites (temporal)?
...
### --> ? show that obsrvations,e.g., diff times of day (when we don't always have observations) are correlated, (?) adjusted for site. 

Hoek et al. (2008) review of LUR approaches cites Sebret et al.'s (2000) work. saw correlations between four (seasonal) 14-dy sampling ranging form 0.63-0.98
...
aqs_ufp <- annual_ufp_and_cov %>%
  ...
  left_join(unique(mm_stops[c("site_id", "aqs_id", "aqs_location")])) %>%
...
for(i in seq_along(annual_names)) {
  ...
  mymap <- suppressMessages(
     all_annual %>% map_fn(color_by = annual_names[i], outline_points = F,
...
## Prediction differences for ACT cohort
...
annual_names2 <- setdiff(annual_names, c("primary_ufp" 
                                         #"windsorize_ufp"
                                         ))

# min and max difference of all estimates
diff_range <- pred_differences %>%
  select(annual_names2) %>%
  dplyr::summarize(min = min(.),
                   max = max(.)) %>%
  round()
...
legend_breaks <- c(diff_range$min, 0, diff_range$max)

for(i in seq_along(annual_names2)) {
  #i=3
  pred_differences$color_by <- pred_differences[[annual_names2[i]]]
  ...
  names(difference_maps)[i] <- annual_names2[i]
}

ggpubr::ggarrange(plotlist = difference_maps,
          ncol = 3, nrow = 2,
          common.legend = T, legend = "right",
          labels = annual_names2 
          ) %>%
...
# saveRDS(all_annual, file.path("Data", "Aim 2", "Mobile Monitoring", "mm_annual.rda"))
