...
ufp_windsorized <- ufp %>%
  group_by(site_id) %>%
  mutate(
    ufp_wind = ifelse(ptrak > quantile(ptrak, (1-trim_quantile), na.rm = T), quantile(ptrak, (1-trim_quantile), na.rm = T),
...
####  --> ? or take annual_ufp_and_cov & exponentiate if contains("log_"), rename proximity vars? 

```{r}
# # use native scale transformations
# 
...
stop_mean_results <- estimate_annual_avg(dt = ufp_mean)
summary(stop_mean_results$annual_estimates)
...
```{r}
trim10_results <- estimate_annual_avg(dt = ufp_trim10)
...
```{r}
ufp_windsorized_results <- estimate_annual_avg(dt = ufp_windsorized)
...
ufp_uw_results <- estimate_annual_avg(dt = ufp_windsorized, 
                                      lm_x_names = "site_id")
