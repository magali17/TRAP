...
### --> ? drop AQS sites that are not representative of ACT cohort (will be using site_id for annual avg prediction model)? so plots and tables will be representative of data we are actually using to estimate annual averages
...
### --> make stop_type boxplots next to eachother?
...
p.tod5 <- ufp %>% 
  #mutate(hour = factor(hour)) %>%
  ggplot(aes(x=stop_type, y=ptrak,  fill=tod5)) + 
  geom_boxplot() +
  labs(x = "Stop type",
       y = "",
       fill = "Time of day") 
...
### --> ? Drop dissimilar AQS sites before proceding w/ rest of analysis (tables, plots, regression)
...
![Map of poorly performing sites. Gray = stop with high inter-method variation. Green = high LUR residual (above `r myquant` quantile). Other colored dots are UFP concentrations estimates at all sites - darker red indicates higher concentrations. "MC" locations indicate AQS sites.](../Write Up/1. Proposal/Aim 2. 2019 UFP/Images/resid_yhat_s_tow2_tod2_q0.98.png)


### --> get list of what all covariates mean eactly  (e.g., evergreen)
### --> ? 3D/correlation plot of most correlated covariates?
...
annual_ufp_and_cov %>% 
  map_fn(map_title = "Annual average UFP concentrations \nat mobile monitoring locations \nbased on yhat_s_tow2_tod5")

# height <- max(annual_ufp_and_cov$latitude) - min(annual_ufp_and_cov$latitude)
# width <- max(annual_ufp_and_cov$longitude) - min(annual_ufp_and_cov$longitude)
# 
# # make map boundaries little larger than data dimensions
# bbox <- c(
#   min(annual_ufp_and_cov$longitude) - 0.2*width,
#   min(annual_ufp_and_cov$latitude) - 0.1*height,
#   max(annual_ufp_and_cov$longitude) + 0.2*width,
#   max(annual_ufp_and_cov$latitude) + 0.1*height
# )
# 
...
# ggmap(ggmap = map, darken = c(0.5, "white")) + theme_void() + 
#    geom_point(data = annual_ufp_and_cov, 
#              aes(x = longitude, y = latitude), col= "black", 
#              size = 2.5, 
#              ) +
#   geom_point(data = annual_ufp_and_cov, 
#              aes(x = longitude, y = latitude, col = ufp), 
#              alpha = 0.8, size = 2, 
#              ) +
#   scale_color_gradient(name = "pt/cm3", low = "yellow", high = "red")  +
#   theme(legend.position = c(0.98, 0.01), legend.justification = c(1,0)) +
#   labs(title = "Map of annual average UFP concentrations at mobile monitoring locations")
  

```

Map using unweighted annual averegae estimates. Spatial patterns are nearly identical.

```{r}

suppressWarnings(left_join(site_locations, preds.uw, by= "site_id") %>%
  rename(ufp = contains("yhat")) %>%
  left_join(cov_mm, by= "site_id")  %>%
  drop_na()) %>% 
  map_fn(map_title = "Annual average UFP concentrations \nat mobile monitoring locations \nbased on unweighted means")
...
annual_ufp_and_cov %>%
  ...
  kable(caption = "Annual average UFP concentrations to be used in UK", 
        col.names = c("Stop kind", "N", "Mean (SD)", "Median (IQR)", "Min", "Max")
        ) %>%
  kable_styling()
