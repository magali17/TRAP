...
### --> summarize all sensitivity analyses 
### --> ? don't include native scale? 

```{r}
join_dfs_by <- c("site_id", "yhat")
  
all_annual <- annual_ufp_and_cov %>% 
  select(site_id, latitude, longitude, 
         primary_annual = ufp) %>%
  #native scale
  left_join(annual_ufp_and_cov_native_scale[,c("site_id", "ufp")], by = "site_id") %>%
  rename(native_scale_annual = ufp) %>%
  # stop means
  left_join(stop_mean_results$annual_estimates[,join_dfs_by], by = "site_id") %>%
  rename(means_annual = yhat) %>%
  # trim 10%
  left_join(trim10_results$annual_estimates[,join_dfs_by], by = "site_id") %>%
  rename(trim10_annual = yhat) %>%
  # windsorized
  left_join(windsorized_results$annual_estimates[,join_dfs_by], by = "site_id") %>%
  rename(windsorize_annual = yhat) %>%
  # windsorized
  left_join(uw_results$annual_estimates[,join_dfs_by], by = "site_id") %>%
  rename(uw_annual = yhat)  

annual_names <- names(all_annual)[grepl("annual", names(all_annual))]

all_annual_l <- all_annual %>% gather("analysis", "prediction", contains("annual")) %>%
  mutate(analysis = str_replace(analysis, "_annual", ""),
...
all_annual %>%
  gather("analysis", "prediction", contains("annual"), -contains("primary")) %>%
  mutate(analysis = str_replace(analysis, "_annual", "")) %>% 
  ggplot(aes(x = primary_annual, y = prediction, group = analysis, col = analysis)) + 
