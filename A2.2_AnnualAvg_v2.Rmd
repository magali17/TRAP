...
#load data
stops0 <- readRDS(file.path("Data", "Aim 2", "Mobile Monitoring", "mm_stops_2020-10-02.rda")) 
...
cov_mm <- readRDS(file.path("Data", "Aim 2", "Geocovariates", "cov_mm_preprocessed.rda")) %>%
  # drop time-varying pop density, NDVI, emissions 
...
# Sample Size - All stops

Number of median stop concentration estimates (median_all_data).

```{r}
stops_l0 <- stops %>%
  select(date, site_id, contains("median_all_data"), hour:tod2) %>%
  gather(Pollutant, value, contains("all_data")) %>%
  #select(-value) %>%
  mutate(Pollutant = ifelse(grepl("bc", Pollutant), "BC", "UFP")) %>%
  drop_na()

#first_hr <- min(stops_l0$hour[stops_l0$hour != min(stops_l0$hour)])
#last_hr <- min(stops_l0$hour)
...
n_tod5 <- stops_l0 %>%
  #filter(grepl("UFP", Pollutant)) %>%
  group_by(Pollutant, site_id, tod5) %>%
  dplyr::summarize(N = n()) %>%
  group_by(Pollutant, Time=tod5) %>%
  distribution.table(var.string = "N") %>%
  ungroup() %>%
  mutate(
...
n <- rbind(n_tot, n_season, n_tow, n_tod5)
...
n %>%
  ...
  add_row(Time = "Hour", .before = 10) %>%
    kable(caption = "Number of UFP samples per site overall, by season and by hour of the day. N = unique number of sites visited") %>%
  add_indent(c(3:6, 8:9, 11:15)) %>%
  kable_styling() 
...
n %>%
  ...
  add_row(Time = "Time of Week", .before = 7) %>%
  add_row(Time = "Hour", .before = 10) %>%
    kable(caption = "Number of BC samples per site overall, by season and by hour of the day. N = unique number of sites visited") %>%
  add_indent(c(3:6, 8:9, 11:15)) %>%
  kable_styling()
...
stops_l_all_analyses %>%
  ...
  kable(caption = "Distribution of stop concentrations for all the data, the primary analysis and sensitivity analyses") %>%
  kable_styling()
...
# all data & primary
stops_l_all_analyses %>% 
  filter(Estimate %in% c("median_all_data", "median_trim5")) %>%
  mutate(Estimate = recode_factor(factor(Estimate),
                       "median_all_data" = "All Data",
                       "median_trim5" = "5% Trim"
                       )) %>%
    ggplot(aes(x=Estimate, y=value, fill=Estimate)) + 
  geom_boxplot() + 
  facet_wrap(~Pollutant, scales="free", ncol=1) + 
  labs(title = "Distribution of stop concentrations for all the data, the primary analysis and sensitivity analyses",
       y = "Stop Concentration",
       x = "Data Used"
       )  + 
...
A similar number of stop samples were collected each season and day of the week for each pollutant. Some morning, afternoon & evening hours were sampled more frequently than others.
...
stops_l %>%
  ...
  labs(
    title = "Number of stop samples each season, day of the week and hour",
    x = ""
    # y = ""  
  )
...
```{r}
stops_l %>%
  mutate(site_no = factor(site_no, levels =  sort(unique(stops_l$site_no))),
         hour = factor(hour, levels =  sort(unique(stops_l$hour)))) %>%
  ungroup() %>%
    label_pollutant(var = "Pollutant", label = "pollutant") %>%
  ggplot(aes(y=hour, x=site_no)) + 
  stat_bin2d() + 
  facet_wrap(~Pollutant, ncol=1) + 
  labs(title = "Number of samples per site-hour",
       x = "Site",
       y = "Hour"
       )
...
conc_distributions %>%
  ...
  add_row(time = "Time of Week", .before = 7) %>%
  add_row(time = "Hour", .before = 10) %>%
  kable(caption = "Distribution of median UFP stop concentrations (pt/cm3) over time") %>%
  add_indent(c(3:6, 8:9, 11:31)) %>%
  kable_styling()
...
conc_distributions %>%
  ...
  add_row(time = "Time of Week", .before = 7) %>%
  add_row(time = "Hour", .before = 10) %>%
  kable(caption = "Distribution of median BC stop concentrations (ng/m3) over time") %>%
  add_indent(c(3:6, 8:9, 11:31)) %>%
  kable_styling()
...
### --> ? Compare temporal trends of sites w/ ~5 TOD visits vs those w/ ~2 TODs?
### --> boxplots of ufp vs hour for a few sites - show that conc's vary drastically within a site for any given hour (~ANOVA)
...
Most (~73%) of the variation in UFP stop concentration occurs __[?]within stops at any particular time__. About 22% of the variation in UFP concentration occurs between stops and only ~ 4% occurs as a result temporal changes.
...
### --> interaction regressions: create site type: based on rd proximity, airport prox, pop, ndvi? interact: site_type*TOD, site_type*TOW
-harder to do b/c will have to e.g., weigh each category of e.g., road proximity differently, based on what data are available for calcualting annual averages
### ? are interactions significant? how do the annual averages compare? 

### also? use PLS, divide high vs low scores to create site_type A vs B
...
#season-day-tod5
preds_s_tow7_tod5_ufp0 <- stops %>% ungroup() %>%
...
preds_s_tow7_tod5_bc0 <- preds_s_tow7_tod5_ufp0

#season-day-hr
preds_s_tow7_tod21_ufp0 <- stops %>% ungroup() %>%
  tidyr::expand(site_id, season, day, hour) %>%
  mutate(season_wt = 1/n.seasons.sampled,
         day_wt = 1/7,
         tod21_wt = ifelse(test= hour %in% c(23, 0,4,5), 
                           yes =  1.75/24, 
                           no= 1/24),
         wt = season_wt*day_wt*tod21_wt)
preds_s_tow7_tod21_bc0 <- preds_s_tow7_tod21_ufp0
...
fit_s_tow7_tod5_ufp <- update(fit_s_tow2_tod5_ufp, ~. -time_of_week + factor(day, ordered = F))
fit_s_tow7_tod5_bc <- update(fit_s_tow7_tod5_ufp, subset = (Pollutant =="bc_ng_m3"))

fit_s_tow7_tod21_ufp <- update(fit_s_ufp, ~. + factor(day, ordered = F) + factor(hour))
fit_s_tow7_tod21_bc <- update(fit_s_tow7_tod21_ufp, subset = (Pollutant =="bc_ng_m3"))
...
```{r}
# season-day-TOD5
preds_s_tow7_tod5_ufp <- preds_s_tow7_tod5_ufp0 %>%
  mutate(yhat = predict(object = fit_s_tow7_tod5_ufp, newdata = .),
         yhat_wt = yhat*wt) %>%
  group_by(site_id) %>%
  dplyr::summarize(sum_yhat_wt = sum(yhat_wt)) %>%
  mutate(Pollutant = "ufp_pt_cm3")

preds_s_tow7_tod5_bc <- preds_s_tow7_tod5_bc0 %>%
  mutate(yhat = predict(object = fit_s_tow7_tod5_bc, newdata = .),
         yhat_wt = yhat*wt) %>%
  group_by(site_id) %>%
  dplyr::summarize(sum_yhat_wt = sum(yhat_wt)) %>%
  mutate(Pollutant = "bc_ng_m3")

preds_s_tow7_tod5 <- rbind(preds_s_tow7_tod5_ufp, preds_s_tow7_tod5_bc) %>%
  rename(regression_s_tow7_tod5 = sum_yhat_wt)

```

```{r}
# season-tow7-TOD21
preds_s_tow7_tod21_ufp <- preds_s_tow7_tod21_ufp0 %>%
  mutate(yhat = predict(object = fit_s_tow7_tod21_ufp, newdata = .),
         yhat_wt = yhat*wt) %>%
  group_by(site_id) %>%
  dplyr::summarize(sum_yhat_wt = sum(yhat_wt)) %>%
  mutate(Pollutant = "ufp_pt_cm3")

preds_s_tow7_tod21_bc <- preds_s_tow7_tod21_bc0 %>%
  mutate(yhat = predict(object = fit_s_tow7_tod21_bc, newdata = .),
         yhat_wt = yhat*wt) %>%
  group_by(site_id) %>%
  dplyr::summarize(sum_yhat_wt = sum(yhat_wt)) %>%
  mutate(Pollutant = "bc_ng_m3")

preds_s_tow7_tod21 <- rbind(preds_s_tow7_tod21_ufp, preds_s_tow7_tod21_bc) %>%
  rename(regression_s_tow7_tod21 = sum_yhat_wt)
...
stops_l <- stops_l %>%
  left_join(preds_s_tow7_tod21) %>%
  left_join(preds_s_tow7_tod5) %>%
...
method_names <- setdiff(method_names_all, c("regression_s_tow2_tod5", "regression_s_tow7_tod5", "regression_s_tow7_tod21"))
...
annual.l <- annual %>%
  ...
  mutate(
    ...
    weight = factor(weight, levels = c("uw", "s", "s_tow2", "s_tow2_tod2", "s_tow2_tod5", "s_tow7_tod5", "s_tow7_tod21"))
  ) %>%
  drop_na(value)
...
### --> ? why is tod2_no_sites = 25 & not 21? (13+18)

```{r}
annual_subset_tod2 <- annual %>%
  select(-contains(c("tow7", "tod5"))) %>%
  #select(-c(regression_s_tow7_tod5, regression_s_tow2_tod5)) %>%
  drop_na(
    ## "site_only_s_tow2 not found" knit error??
    site_only_s_tow2, 
    site_only_s_tow2_tod2) %>%
  gather(method_weight, value, contains(c("site_only", "regression"))) %>%
  mutate(
    method = ifelse(grepl("site_only", method_weight), "site only", "regression"),
    weight = ifelse(grepl("site_only", method_weight), substr(method_weight, 11, nchar(method_weight)), substr(method_weight, 12, nchar(method_weight))),
    weight = factor(weight, levels = c("uw", "s", "s_tow2", "s_tow2_tod2" #"s_tow2_tod5", "s_tow7_tod5"
                                       )))
...
annual_subset_tow2 <- annual %>%
      label_pollutant(var = "Pollutant", label = "pollutant") %>%
  select(-contains(c("tod"))) %>%
  # select(-c(site_only_s_tow2_tod2,
  #           regression_s_tow7_tod5, regression_s_tow2_tod5, regression_s_tow2_tod2)) %>%
...
```{r}
data.frame(
  Analysis =  c("primary", "stop_means", "trim10", "windsorize", "uw", "tow7_tod21")) %>%
...
# finer temporal adjustment: day (7) & hour (21/24)
  left_join(estimate_annual_avg(dt = stops, lm_x_names = c("site_id", "season", "day", "hour"),
                                       var = "ufp_pt_cm3", estimate_label = "ufp_tow7_tod21"))
...
# finer temporal adjustment: day (7) & hour (21/24)
  left_join(estimate_annual_avg(dt = stops, lm_x_names = c("site_id", "season", "day", "hour"),
                                       var = "bc_ng_m3", estimate_label = "bc_tow7_tod21"))
...
# long format
annual_lat_long_l <- annual_lat_long %>%
  gather("Analysis", "value", contains(c("ufp", "bc"))) %>%
...
# dimensions of data 
height <- max(annual_lat_long_l$latitude) - min(annual_lat_long_l$latitude)
width <- max(annual_lat_long_l$longitude) - min(annual_lat_long_l$longitude)

# make map boundaries little larger than data dimensions
bbox <- c(
  min(annual_lat_long_l$longitude) - 0.5*width,
  min(annual_lat_long_l$latitude) - 0.1*height,
  max(annual_lat_long_l$longitude) + 0.5*width,
  max(annual_lat_long_l$latitude) + 0.1*height
)

names(bbox) <- c("left", "bottom", "right", "top")

# Make a map base layer of "Stamen" tiles
map <- suppressMessages(get_stamenmap(bbox = bbox, zoom = 11,
                                      maptype = "terrain"))

# Make basic map image from the tiles
map0 <- ggmap(ggmap = map, darken = c(0.5, "white")) + theme_void()
...
map0 +
  geom_point(data = subset(annual_lat_long_l, 
  ...
  labs(title = "UFP Primary Analysis"
       )
...
# UFP
map0 +
  geom_point(data = subset(annual_lat_long_l, subset = grepl("UFP", Pollutant)),
             aes(x = longitude, y = latitude, col = value),
             alpha = 0.8,
             ) +
  scale_color_gradient(name = "pt/cm3", 
                       low = "yellow", 
                       high = "red") +
  facet_wrap(~Analysis, labeller = "label_both") + 
  theme(legend.position = c(0.98, 0.01), legend.justification = c(1,0))  + 
  labs(title = "UFP Annual Averages for Sensitivit Analyses"
...
map0 +
  ...
  scale_color_gradient(name = "ng/m3", 
                       low = "yellow", 
                       high = "red") +
  theme(legend.position = c(0.98, 0.01), legend.justification = c(1,0))  + 
  labs(title = "BC Primary Analysis"
       )
...
map0 +
  ...
  facet_wrap(~Analysis, labeller = "label_both") + 
  theme(legend.position = c(0.98, 0.01), legend.justification = c(1,0))  + 
  labs(title = "BC Annual Averages for Sensitivit Analyses"
...
# saveRDS(annual, file.path("Data", "Aim 2", "Mobile Monitoring", "annual_2020-10-02.rda"))
# 
# # short dataset for GIS
# stops %>%
#   select(site_id, latitude = site_lat, longitude = site_long, route) %>%
#   unique() %>%
#   right_join(annual) %>%
#   write.csv(., file.path("Data", "Aim 2", "Mobile Monitoring", "annual_2020-10-02_loc&avg.csv"))
