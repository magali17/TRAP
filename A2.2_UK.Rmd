...
pacman::p_load(dplyr, tidyverse, knitr, kableExtra, Himsc, EnvStats, lubridate, glmnet, ggpubr, VCA,
...
#105 locations w/ samples each month #nrow(m.same_months)
...
```{r, fig.height=8}
h.l.tow2 %>%
  ggplot(aes(x=site_no, y= ptrak_ct_cm3, fill=factor(diff_lvl), group=site_no)) + 
  geom_boxplot() +
...
Correlations. Using season-TOW2 estimates (n = `nrow(tow2)`).
...
Evaluate predictions using different methods.
...
yhat_mean_sea_tow2_plot <- colo.plot(data.wide = tow2, 
                    x.variable = "mean_sea_tow2", 
                    y.variable = "yhat_mean_sea_tow2"
                    )

yhat_mean_sea_plot <- colo.plot(data.wide = tow2, 
                    x.variable = "mean_sea", 
                    y.variable = "yhat_mean_sea"
                    )

yhat_mean_uw <- colo.plot(data.wide = tow2, 
                    x.variable = "mean_uw", 
...
Drop the following geocovariates:
* AP predictions
* those with little variation 
* those with extreme values  

[code, Keller 2015] (https://github.com/kaufman-lab/STmodel/blob/master/functions_covariate_preprocess_regional.R)

```{r}
#drop AP predictions


#drop little cov w/ little variation
#?? var(mm.geo)
