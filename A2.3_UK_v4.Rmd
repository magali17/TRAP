---
title: "Aim 2: UK Model using PLS"
...
analysis_names <- str_replace(ufp_names, "ufp_", "")

Analysis_description = c("Primary analysis: using stop medians; trimming 5%; regression to estimate season-, TOW2- and TOD5-adjusted concentrations; log-transformed concentrations and proximity covariates",
...
data.frame(
  ...
  kable(., caption = "Description of analyses", 
        col.names = c("Label", "Analysis")) %>%
  kable_styling()
...
### --> add variogram distance plotted to table (vs fraction)
...
# 
# uk_monitoring_area <- uk_predictions(dt = annual,
#                                   # this changes
#                      cov_loc_new = cov_act_all,
#                       y_name = "primary_ufp",
#                       cov_names. = cov_names_log,
...
### --> update study, ST to not include monitoring area
...
### --> clarify in descriiptions what data this is for (e.g., ACT locations?)
...
act_cov_lasso <- lasso_fn(dt = annual, x_names = cov_names_log, y_name = "primary_ufp", 
              lambda. = .08
              )
