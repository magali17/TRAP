...
annual0 <- readRDS(file.path("Data", "Aim 2", "Mobile Monitoring", "annual.rda")) %>%
  drop_na(m_to_a1)
geo_mm <- readRDS(file.path("Data", "Aim 2", "Geocovariates", "geo.mm.rda")) %>%
...
### --> issue: PLS only works when variables aren't scaled. "errors when variables aren't scaled:...In FUN(X[[i]], ...) : Scaling with (near) zero standard deviation"

Create training,  test and AQS set.

### --> CHECK VARIOG DISTNACEs now that ufp isn't being included in PLS. add to variog_dist_to_plot 

# --> CV R2 and RMSE look too good?  
...
exclude_from_pls <- c("site_id", "ufp", "set", "cv_prediction")

#distances to plot for different PLS components
variog_dist_to_plot <- data.frame(pls_comp = c(c(1:10) #,
                                               #c(11:20)
                                               ),
                                  dist_fraction = c(0.74, 0.3, 0.25, 0.13, 0.13, 
                                                    rep(0.11, 5) #,
                                                    #rep(0.09, 10) #??
                                                    )
                                  )
...
for(i in seq_len(use_n_scores)) {
  ...
  pls_train <- plsr(log_ufp ~.,
               data=annual_train[,!(names(annual_train) %in% exclude_from_pls)], 
               ncomp = i,
               #scale=T
                  )
  ...
  max.plot.dist <- max.dist* cv.eval$dist_fraction[cv.eval$pls_comp==i] #[1] 
...
pls_train_test <- plsr(log_ufp ~.,
             data=annual_train_test[,!(names(annual_train_test) %in% exclude_from_pls)], 
             ncomp = cv_comp,
             #scale=T
                )
...
### --> add variogram parameters: nugget, partial sill, range (m).

```{r}
max.plot.dist <- max.dist*#.2
  variog_dist_to_plot$dist_fraction[variog_dist_to_plot$pls_comp==cv_comp]
...
legend("bottomright",
       legend = c("Empirical", paste0("Residual Model (", "REML", ")")),
       lty=c(1:2), col = c(1:2),
       #cex = 0.7
       )
...
### --> need to clean up ACT covariates; various covariates are labeled are differently in old act dataset  
