...
# # SENSITIVITY ANALYSIS: native scale proximity covariates
...
rmse_inx <- cv.eval$RMSE == min(cv.eval$RMSE)
...
### --> ? does this help?? only for plot()? locations.inside(interpolation_grid, act_predictions$borders)
#             #https://www.stat.washington.edu/peter/591/geoR_sln.html
...
# --> update w/ new datasets

```{r, results="hide"}
set.seed(1)

# TEST w/ same dataset
## get CV: RMSE, R2, comp no., dist_fract
t <- pls_uk_cv_eval(dt2 = annual_train_test,
                           max_pls_comp = 5,
                           dist_fract = c(0.05, seq(0.1, 0.4, by=0.1)),  # <-- update
...
# example of fn to use
t2 <- uk_predictions(pls_comp = t$cv_table$PLS_Components,
  variogram_dist_fract = t$cv_table$Variogram_Distance_Fraction
  )
...
Table 
e.g. model, distribution of predictions, CV R2, RMSE, 
```{r}
