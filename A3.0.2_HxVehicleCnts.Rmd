...
Using a linear fit & an interaction term for each county to allow each county to have its own slope:
...
ef %>%
  ...
  ggplot(aes(x=Year, y = g_per_m, col=Pollutant, shape = Interpolated)) + 
  geom_point(size=2) +
  geom_line() + 
  facet_wrap(~Pollutant, scales = "free") +
  labs(y = "Emission Factor (g/m)",
...
ggarrange(plotlist = plot_list, 
  ...
  annotate_figure(top = paste0("AADT on 100 m road segments before and after smoothing (n = ", round_to, " nearby segments)"), 
                  bottom = "Before smoothing: single segment AADT", 
                  left = paste0("After smoothing: ", round_to, " segment median AADT")
                    )
...
ggarrange(plotlist = plot_list, 
          ncol = 3, nrow = 2,
          common.legend = T, legend = "right") %>%
  annotate_figure(top = paste0("Emissions (g) on 100 m road segments before and after smoothing (n = ", round_to, " nearby segments)"), 
...
map_title <- ifelse(grepl("original", pollutant_names), 
                      paste0(substr(pollutant_names, 6, 9), " original"),
                      paste0(substr(pollutant_names, 6, 9), " smooth")
                      )
