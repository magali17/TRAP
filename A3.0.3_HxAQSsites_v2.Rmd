...
# Map of AQS sites used for trend adjustment and/or validation
...
pscaa_bc0 <-  data.table::fread(file.path(folder_path, "PSCAA", "BC_daily.csv"),
...
pscaa_bc6330 <-  data.table::fread(file.path(folder_path, "PSCAA", "BC633_daily.csv"),
                           skip = "Observation Time", 
                           fill=T, sep = ",") %>%
  #only keep relevant sites
  select_if(grepl(vars_to_get, names(.), ignore.case = T)) %>%
