...
knitr::opts_chunk$set(echo = F, cache=F, cache.comments = F, message = F, warning = F, tidy.opts=list(width.cutoff=60), tidy=TRUE 
                      #fig.height = 8
                      )  
...
map0 +
  ...
  labs(
    title = "AQS sites with historical BC and EC measurements \nin and near the study area",
    fill = "",
    col = ""
    ) 
...
epa_daily %>%
  ...
  geom_bar(data=subset(epa_daily, meets_criteria == TRUE)) +
  geom_bar(data=subset(epa_daily, meets_criteria == FALSE), 
           alpha=0.3,
           show.legend = F) + 
  #scale_alpha_discrete( range = c(0.4, 1)) + 
  geom_hline(yintercept = min_day_criteria, linetype = 2, alpha=0.5) +
  scale_y_continuous(breaks = c(seq(0, 125, 25),  min_day_criteria)) +
...
m1 <- aqs_trend %>%
  ...
  drop_na(mean) %>%
  # splines::ns(Year, knots = c(2005, 2012), Boundary.knots = c(1997, 2018))
  lm(as.formula(spline_fit), data = .)
  
aqs_trend$yhat <- predict(m1, newdata = aqs_trend) #%>% exp()

#calculate the relative difference between EC for any given year, relative to the reference year (~latest year), on an additive and multiplicative scale
trend_ref <- 2019
...
aqs_trend %>%
  ...
  mutate(year_type = ifelse(Year ==trend_ref, "Reference", "Adjusted"
  ...
  scale_y_continuous(breaks = c(seq(0,2.4, 0.2))) +
  labs(
    title = "Annual adjustment factors based on the EC trend at Beacon Hill",
...
pscaa %>%
  ...
  labs(
    title = "Number of daily average readings per year and site",
    y = "Count"
  ) +
scale_shape_manual(values = 1:length(unique(pscaa$Site))) +
...
Unique site-years in original dataset (includes Duwamish & all years): `r nrow(unique_yr_site)`
...
pscaa_yearly %>%
  ...
  labs(title = "BC readings (ng/m3) used for validation",
       y = "BC (ng/m3)"
       )  
...
with the trend adjustment 

```{r}
aqs_trend %>%
  #convert from ug/m3 to ng/m3
  mutate(mean = mean*1000) %>%
  gather("method", "value", ratio #difference:ratio
         ) %>%
  ggplot(aes(x=Year, y = value)) + 
  geom_point(alpha=0.4) + 
  geom_line(aes(group=method, linetype=method), alpha=0.4) +
  #don't include duwamish
  geom_point(data=subset(pscaa_yearly, !grepl("Duwamish", Site)), 
             aes(y=mean, col = Site, shape=Site)) + 
  geom_line(data=subset(pscaa_yearly, !grepl("Duwamish", Site)), 
            aes(y=mean, col = Site)) +
  scale_shape_manual(values =  seq_along(unique(pscaa_yearly$Site)) ) + 

  labs(
    title = "BC readings (ug/m3) used for validation \nand temporal trend adjustment factors",
    linetype = "Trend\nAdjustment\nMethod"
    )  
 
```

alternative - 2D hist of available data 
...
pscaa_yearly %>%
  ...
  #don't include duwamish
  filter(!grepl("Duwamish", Site)) %>%
  ggplot(aes(x=Year, y=Site, fill=mean)) + 
  geom_tile() +
  #stat_bin2d() + 
...
pscaa_yearly %>%
  ...
  labs(title = "Annual Averages by year",
       y = "BC (ng/m3)"
...
aqs_p1 <- pscaa_yearly %>%
  ...
  group_by(Year) %>%
  distribution.table(var.string = "mean",  
                     ) 
...
pscaa_yearly %>%
  ...
  kable(caption = "Distribution of annual averages (ng/m3) for each year with observationss. N = number of AQS sites.") %>%
  kable_styling()
```
...
pscaa_yearly %>%
  ...
  labs(title = "Annual Averages by site",
       y = "BC (ng/m3)"
...
pscaa_yearly %>%
  ...
  #convert from ug/m3 to ng/m3
  mutate(mean = mean*1000) %>%
  group_by(Site = "Overall") %>%
  distribution.table(var.string = "mean")  %>%
  rbind(aqs_p0) %>%
  kable(caption = "Distribution of annual averages (ng/m3) for each AQS site. N = number of annual averages available for that site.") %>%
  kable_styling()
