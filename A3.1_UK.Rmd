...
aqs_emissions <-  read_sf(file.path("..", "GIS", "Shapefiles", "AQS Sites", "aqs_nearby.shp")) %>%
  st_drop_geometry() %>%
...
grid <- grid_cov %>% #left_join(
  #datasets are arranged in same order
...
### --> check that these years are correct. DROP 2006??
ndvi_data_yrs <- c(2006) #c(1990:1993, 2006, 2010, 2019)
...
mm_og_scores <- mm_scores0 %>%
  ...
  mutate(Location = "Mobile Monitoring",
         ...
         #Pollutant = toupper(Pollutant)
                      ...
                      ### DONE. # --> replace w/ cohort_scores0
score_comp <- cohort_scores0 %>% #grid_scores0 %>%
  select(
    Pollutant,
    #space1, space2, 
