...
### -->  spatial PM2.5 variation change over time analysis. Are the spatial contrasts conserved overtime? if trends are generally conserved, could justify this aim? could just use a trend adjustment for long-term exposure assessment?

* ? apply a different trend?
* update model notation   
* Waiting for new data to update the script   
  - new TVCs for all existing datasets   
  - new TVCs for new datasets: extended AQS sites, new grid   
* update PLS component numbers & UK plotting distance parameters after fit final Aim 2 model
...
### --> add model w/ just TVCs (& just space covariates?). compare these predictiosn to primary model. If predictions are not that different, TVCS/space cov don't add much to model predictions?
...
## --> update using grid_trap_sum.csv when have other covariates
emissions_grid <- read.csv(file.path("Data", "Aim 3", "Emissions", "grid_old_trap_sum.csv")) %>%
  select(-gid) %>%
  rename(site_id = native_id) %>%
  #?? take out "_" before buffer - for fn later
  rename_if(grepl("_g_", names(.)), 
...
# emissions_cohort <- read.csv(file.path("Data", "Aim 3", "Emissions", "______.csv")) %>%
#   select(-gid) %>%
#   rename(site_id = native_id)


### --> update native_id when get other geocovariates
      #### delete this dataset when rename native_id
aqs_monitors_nearby_TEMP <- read.csv(file.path("Data", "Aim 3", "Hx BC at AQS Sites", "EPA", "aqs_monitors_nearby_bc.csv"))
## add stop IDs to sites where we stopped 
aqs_monitors_nearby_TEMP$site_id[grepl("10th & Weller", aqs_monitors_nearby_TEMP$Local.Site.Name)] <- "MC0120"
aqs_monitors_nearby_TEMP$site_id[grepl("Beacon Hill", aqs_monitors_nearby_TEMP$Local.Site.Name)] <- "MC0003"
aqs_monitors_nearby_TEMP$site_id[grepl("Beacon Hill", aqs_monitors_nearby_TEMP$Local.Site.Name)] <- "MC0003"
aqs_monitors_nearby_TEMP$site_id[grepl("Duwamish", aqs_monitors_nearby_TEMP$Local.Site.Name)] <- "MC0126"
aqs_monitors_nearby_TEMP$site_id[grepl("Kent", aqs_monitors_nearby_TEMP$Local.Site.Name)] <- "MC0406"
aqs_monitors_nearby_TEMP$site_id[grepl("Tukwila", aqs_monitors_nearby_TEMP$Local.Site.Name)] <- "MC0002"
...
emissions_aqs_nearby <- read.csv(file.path("Data", "Aim 3", "Emissions", "aqs_trap_sum.csv")) %>%
  select(-gid) %>%
  rename(site_id = native_id)
...
# AQS sites (including those outside study area)
### --> upload
...
# grid in study area (from GIS)
                                        ## used to have grid_200602.csv but this doesn't have native_id
grid_in_study <- read.csv(file.path("..", "GIS", "Shapefiles", "Predictions", "grid", "grid_mm.csv")) %>%
  filter(study == TRUE) %>%
  select(site_id = native_id)
  #select(location_id = location_i)

grid_in_study <- grid_in_study$site_id





### --> ERROR: native_id is diff for cov_grid_preprocessed.rda vs emissions_grid

#emissions_grid$site_id %>% as.numeric()
...
### --> change to seq_along(dt_list) when get EC cohort estimates (for() loop) here & for nox
...
### --> replace all of grid_scores0 w/ cohort_scores0 once get all TVCs for cohort?
...
grid_scores0 %>%
  ...
  distribution.table(var.string = "value", round.int = 1) %>%
  kable(caption = "Distribution of PLS scores for the grid, by pollutant and TVC") %>%
  kable_styling()
```
...
### --> replace grid w/ cohort predictions & change Location description to "Cohort"
...
mm_og_scores <- mm_scores0 %>%
  ...
  gather("TVC", "value", c(pop2010, ndvi2006, ec2019,
                           # sensitivity
                           nox2019
                           )) %>%
  separate(col = TVC, 
...
### --> replace w/ cohort_scores0
score_comp <- grid_scores0 %>%
  select(
    Pollutant,
    #space1, space2, 
  ...
  mutate(
    Pollutant = toupper(Pollutant),
    TVC = toupper(TVC),
    Location = factor(Location, levels = c("Mobile Monitoring", "Grid"))
...
```{r}
# score_comp %>%
#   filter(grepl("ec", TVC, ignore.case = T),
#          Year >=1995
#          ) %>%
...
#     alpha=0.2
#     ) +
#   facet_grid(Location~Pollutant,
#              scales="free"
#              ) +
#   scale_x_log10() +
#   labs(x = "PLS Score",
#        title = "Distribution of EC Emissions PLS scores at the mobile monitoring locations used to build the models and GRID/COHORT locations where predictions are made"
#        )


# #pop
# score_comp %>%
#   filter(grepl("pop", TVC, ignore.case = T)) %>%
#   ggplot(data=., aes(x=value,
#                      fill=Year,
#              )
#          ) +
...
#   scale_x_log10() +
#   labs(x = "PLS Score",
#        title = "Distribution of population density PLS scores at the mobile monitoring locations used to build the models and GRID/COHORT locations where predictions are made"
#        )


# #ndvi
# score_comp %>%
#   filter(grepl("ndvi", TVC, ignore.case = T)) %>%
#   ggplot(data=., aes(x=value,
#                      fill=Year,
#              )
#          ) +
...
#   scale_x_log10() +
#   labs(x = "PLS Score",
#        title = "Distribution of NDVI PLS scores at the mobile monitoring locations used to build the models and GRID/COHORT locations where predictions are made"
#        )
...
grid_scores0 %>%
  ...
  labs(
    title = "Grid population density PLS scores over time",
    x = "1990 Census",
    y = "PLS Score",
    linetype = ""
  )
...
scores_high_residual <- grid_scores0 %>%
  filter(Pollutant == "bc") %>%
  mutate(
    #lm_prediction = ifelse(Pollutant == "bc", predict(pop_m1$bc), predict(pop_m1$ufp)),
    lm_residual = ifelse(Pollutant == "bc", residuals(pop_m1$bc), residuals(pop_m1$ufp)),
...
map0 +
  ...
  geom_polygon(data = monitoring_area,
                ...
                alpha = 0.1) +
  geom_point(data = scores_high_residual,
  ...
  labs(
    title = "Grid sites with large residuals (using the BC model)",
    fill = "",
    col = ""
    ) 
...
### --> Show how TVCs have changed over time
...
### --> UPDATE when get cohort covars. replace next line w/ one below it
...
#ufp_cohort_scores1 <- interpolate_ec(dt = ufp_cohort_scores)  
...
grid_scores %>%
  gather("TVC", "value", c(paste0("pop", c(census_data_yrs, last_acs_yr)), 
                           paste0("ndvi", ndvi_data_yrs),
                           paste0("ec", emissions_data_yrs)
                           ) 
         ) %>%
...
grid_scores %>%
  gather("TVC", "value", c(paste0("ec", emissions_data_yrs),
  ...
  group_by(Pollutant, TVC, Year) %>%
  distribution.table(var.string = "value", round.int = 1) %>%
  kable(caption = "Distribution of emission PLS scores for the GRID, by pollutant and TVC") %>%
  kable_styling()
...
```{r}
# # ### --> FIX? pop boxplots form 2010-2019 should be all generally increasing. They look strage at the Q0.75 & 1.5*IQR & outliers to the right
# # ? Are PLS scores increasing and moving from e.g., 75th quantile to the line above the 75th quantile and from the 1.5*IQR becoming outliers? since IQR is becoming narrower - not sure why Q75 isn't going up as much - is there a jump in concentrations?
# 
# 
# adj <- 0.02
# 
...
#   geom_boxplot() + 
#   labs(title = "Example of rnorm(0,1) increased by X%")
...
ggplot(aes(x=Year, y=value, )) + 
  ...
  geom_smooth(se=F) +
  facet_grid(tvc~Pollutant, scales="free") + 
  labs(
    title = "Distribution of observed and interpolated TVC PLS scores for the GRID"
    #fill = "Estimate\nType"
  )  
...
```{r}
#nox
nox_p_interpolated <- grid_scores %>%
...
ggplot(aes(x=Year, y=value, )) + 
  ...
  labs(
    title = "Distribution of observed and interpolated emission TVC PLS scores for the GRID"
    #fill = "Estimate\nType"
  )  
...
grid_scores %>%
  ...
  #distribution.table(var.string = "value", round.int = 1) %>%
  kable(caption = "Distribution of TVC PLS scores for the GRID", 
        digits = 2
        ) %>%
  kable_styling()
...
grid_scores %>%
  ...
  #distribution.table(var.string = "value", round.int = 1) %>%
  kable(caption = "Distribution of emission TVC PLS scores for the GRID", 
        digits = 2
        ) %>%
  kable_styling()
...
### --> update to cohort dt w/ other TVCs
...
#cohort_scores
grid_scores %>%
  gather("Year", "Value", c(paste0("pop", c(census_data_yrs, last_acs_yr)),
...
scores_first_last_yr %>%
  ...
  labs(title = "Change in PLS score from 1995-2019",
...
scores_first_last_yr %>%
  ...
  kable(caption = "Change in variability of time-varying covariates between 1995 and 2019.", 
...
### --> change differences colors to ?blue/black??
...
### --> change ndvi2006 to ndvi2019 when get new covariates in following models
...
(r2_ndvi <- lm(data = lm_df, bc ~ ndvi2006) %>% 
  summary()
  )
...
(r2_m4 <- update(object = r2_m3, ~ . + ndvi2006) %>% 
  summary() 
)
...
(r2_ndvi <- lm(data = lm_df, ufp ~ ndvi2006) %>% 
  summary()
  )
...
### --> uncomment cohort_predictions... once get new covariates. add nox model

```{r, results="hide"}
# use UK to make perdictions

### --> uncomment cohort_predictions... once get new covariates 
...
### --> change eval=F to true when get cohort covariates
...
### --> uncommnet cohort_uk_trend_predictions... when get cohort covariates
...
### --> chnage grid_uk_trend_predictions to cohort_uk_trend_predictions when get covariates

```{r}
grid_uk_trend_predictions %>%
...
### --> change grid_uk_trend_predictions to cohort_uk_trend_predictions when get covariates

```{r}
#stratified by yr
predictions_t1 <- grid_uk_trend_predictions %>%
  drop_na(uk_trend_p) %>%
  # gather("Trend", "prediction", contains("uk")) %>%
...
# add overall
predictions_t2 <- grid_uk_trend_predictions %>%
  drop_na(uk_trend_p) %>%
  label_pollutant(var = "Pollutant", label = "pollutant") %>%
...
Predictions from the primary and NOx predictor model

```{r}
#bc
grid_uk_trend_predictions %>%
  left_join(grid_uk_trend_predictions_nox) %>%
  filter(Pollutant == "bc") %>%
   colo.plot(x.variable = "uk_trend_p", x.label = "Primary Model", 
             y.variable = "uk_trend_p_nox", y.label = "NOx Predictor Model", 
             mytitle = "Comparison of BC predictions from the \nprimary and NOx predictor model", 
...
grid_uk_trend_predictions %>%
  ...
  filter(Pollutant == "ufp") %>%
   colo.plot(x.variable = "uk_trend_p", x.label = "Primary Model", 
             y.variable = "uk_trend_p_nox", y.label = "NOx Predictor Model", 
             mytitle = "Comparison of UFP predictions from the \nprimary and NOx predictor model", 
             int_digits = 1, slope_digits = 2, 
             r2.digits = 2, #rmse.digits = 1
               )
...
# How does the rank order of the predictions over time?

### --> compare rank order 1995 vs 2019? 

```{r}

```

### --> ? Lasso: what are the largest predictors of what sites saw the largest changes? 
...
### --> could include a 2nd temporal trend adjustment?
...
bc_valid_by_yr %>%
  kable(digits = 2, 
        caption = "NO DUWAMISH. Out-of-sample validation by year. N is the number of sites avaialble for that year. RMSE and R2 (MSE-based) are calculated around the 1-1 line.", 
          ) %>%
  kable_styling()
...
bc_valid_by_yr_nox %>%
  kable(digits = 2, 
        caption = "NOx predictor model. NO DUWAMISH. Out-of-sample validation by year. N is the number of sites avaialble for that year. RMSE and R2 (MSE-based) are calculated around the 1-1 line.", 
          ) %>%
  kable_styling()
