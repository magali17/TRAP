...
get_scores_ec_models <- function( 
  ...
  # scores for model years
  # ec2019_scores <- get_scores(dt = .dt, 
  #                              # no need to change var names here
  #                              rename_vars = FALSE,
  #                              pls_model = .pls_model,
  #                              rename_components = "ec2019") 
  
  # ec names
  df_names <- str_match(names(.dt), pattern = "^ec[0-9]{4}") %>% unique() %>% as.vector()
  # drop "NA"
  ...
  # ec1990_scores <- get_scores(dt = .dt, 
  #                              #var names to change to match model variables (for estimating/predicting scores)
  #                              rename_vars = TRUE, 
  #                              change_var_name = "ec1990_",
  #                              pls_model = .pls_model, 
  #                              rename_components = "ec1990") 
  # 
  # ec1995_scores <- get_scores(dt = .dt, 
  #                              #var names to change to match model variables (for estimating/predicting scores)
  #                              rename_vars = TRUE, 
  #                              change_var_name = "ec1995_",
  #                              pls_model = .pls_model, 
  ...
  # 
  # #combine scores for diff years
  # ec_scores <- cbind(ec1995_scores,                ec2019_scores) 
  
  #ec_scores <- cbind(df, ec2019_scores)
...
### --> change to seq_along(dt_list) when get EC cohort estimates
...
interpolate_ec <- function(dt) {
  ...
  # # interpolate using 5 yr slopes
  # for(i in 1:4){
  #   yr <- 1990+i
  #   var <- paste0("ec", yr)
  #   dt[var] <- dt$ec1990 + dt$ec1990_1995_slope*i
  # }
  # 
  # for(i in 1:4){
  #   yr <- 1995+i
  #   var <- paste0("ec", yr)
  #   dt[var] <- dt$ec1995 + dt$ec1995_2000_slope*i
  # }
  #  
  # for(i in 1:4){
  #   yr <- 2000+i
  #   var <- paste0("ec", yr)
  #   dt[var] <- dt$ec2000 + dt$ec2000_2005_slope*i
  # }
  # 
  # for(i in 1:4){
  #   yr <- 2005+i
  #   var <- paste0("ec", yr)
  #   dt[var] <- dt$ec2005 + dt$ec2005_2010_slope*i
  # }
  # 
  # for(i in 1:4){
  #   yr <- 2010+i
  #   var <- paste0("ec", yr)
  #   dt[var] <- dt$ec2010 + dt$ec2010_2015_slope*i
  # }
...
grid_scores %>%
  ...
  mutate(
    ...
    estimate_type = ifelse((Year %in% c(census_data_yrs, last_acs_yr) & grepl("pop", tvc) ) |
...
### --> is the largest decrease in EC near hwys an artifact of how we calculated EC concentrations? Locations away from hwys had many 0 estimates within buffers and thus small PLS scores to begin with
...
grid_uk_trend_predictions %>%
  ...
  kable(caption = "Pollutant predictions over time at cohort locations, adjusted and not adjusted for a trend") %>%
  kable_styling()
...
bc_valid_p %>%
  ...
  geom_abline(aes(slope = 1, intercept = 0, linetype="1-1")) + 
  #make square plot
  lims(x=c(plot_range_short$min_val, plot_range_short$max_val),
       y=c(plot_range_short$min_val, plot_range_short$max_val)) +
  labs(x= "Observed", 
       y= "Predicted",
       title = "Scatterplot of observed vs predicted BC concentration (ng/m3)",
...
```{r}
## ?? why do I have to reenter this here? relabel rmse fn?
#source("0.Global_Fns.R")
...
# short dataset w/ fewer sies
bc_valid_overall_short <- bc_valid_p %>%
  # ? drop Duwamish?
  filter(str_detect(Site, "Duwamish", negate = T)) %>%
  dplyr::summarize(
    N = n(), #all site-years
...
bc_valid_overall %>%
  kable(digits = 2, 
        caption = "Out-of-sample validation for all AQS sites and years with BC observations (ng/m3). N is the number of site-years. RMSE and R2 (MSE-based) are calculated around the 1-1 line.", 
          ) %>%
  kable_styling()

bc_valid_overall_short %>%
  kable(digits = 2, 
        caption = "NO DUWAMISH. Out-of-sample validation for all AQS sites and years with BC observations (ng/m3). RMSE and R2 (MSE-based) are calculated around the 1-1 line.", 
          ) %>%
  kable_styling()
...
# # NO DUWAMISH
# bc_valid_by_site_short <- bc_valid_p %>%
#   filter(str_detect(Site, "Duwamish", negate = T)) %>%
#   group_by(Site) %>%
#   dplyr::summarize(
#     N = n(),  
...
# bc_valid_by_site_short %>%
#   kable(digits = 2, 
#         caption = "NO DUWAMISH. Out-of-sample validation results for the BC model (ng/m3) by AQS site. N is the number of years avaialble for that site. RMSE and R2 (MSE-based) are calculated around the 1-1 line.", 
#           ) %>%
#   kable_styling()
```
...
bc_valid_by_site %>%
  ...
  # NO DUWAMISH
  geom_hline(aes(yintercept = bc_valid_overall_short$RMSE, linetype="RMSE No Duwamish")) +
    geom_hline(aes(yintercept = bc_valid_overall_short$R2*scale_val, linetype="R2 No Duwamish")) +
...
#   ggplot(aes(x=Site, y=value, shape=Measure, col=N )) + 
#   geom_point() +
#   geom_hline(aes(yintercept = bc_valid_overall_short$RMSE, linetype="RMSE")) +
#   geom_hline(aes(yintercept = bc_valid_overall_short$R2*scale_val, linetype="R2")) +
#   
#   scale_y_continuous(sec.axis = sec_axis(~./scale_val, name = "R2")) + 
#   labs(y = "RMSE (ng/m3)",
#        title = "Out-of-sample validation results for the BC model by AQS site",
...
## NO DUWAMISH
bc_valid_by_yr_short <- bc_valid_p %>%
  filter(str_detect(Site, "Duwamish", negate = T)) %>%
  group_by(Year) %>%
  dplyr::summarize(
    N = n(),  
...
# bc_valid_by_yr %>%
#   kable(digits = 2, 
#         caption = "Out-of-sample validation by year. N is the number of sites avaialble for that year. N is the number of sites avaialble for that year. RMSE and R2 (MSE-based) are calculated around the 1-1 line.", 
#           ) %>%
#   kable_styling()

bc_valid_by_yr_short %>%
  kable(digits = 2, 
