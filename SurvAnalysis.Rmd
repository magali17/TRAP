...
dem <- dem0 %>%
  ...
  mutate(
    ...
    # ?? is this one wrong?? this is 1 yr older than when I calculate it (below)
    ...
    #  age_end_exposure = floor(as.numeric(as.Date(paste0(exposure_year, "-", format(birthdt, "%m-%d"))) - birthdt)/365), 
...
# dem %>%
#     mutate(
#       dementia_now = factor(dementia_now),
#       id_group = ifelse(study_id < 2000, "A",
#                         ifelse(study_id >=2000 & study_id < 3000, "B",
#                         ifelse(study_id >=3000 & study_id < 4000, "C",
#                         ifelse(study_id >=4000 & study_id < 5000, "D",
#                         ifelse(study_id >=5000 & study_id < 6000, "E", "F"
#                         )))))
#       ) %>%
#   filter(
#     id_group == "A",
#          pollutant=="no2") %>%
#   ggplot(aes(x=exposure_year, y=study_id, group= study_id, colour=dementia_now)) +
#   geom_point() +
#   geom_line()  +
#    facet_wrap(~id_group, scales = "free")
...
#number of individuals w/ dementia incidence: ~1269/3472
