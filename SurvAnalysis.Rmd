...
##when using ONSET AGE, need to drop last year of exposure estimates (associated w/ year of last visit)? filter(age_at_end_exposure_year <= age_act) & dementia_now = ifelse(age_at_end_exposure_year < age_act, 0, anydementia),
...
dem <- dem0 %>%
  ...
  mutate(
    ...
    age_at_last_visit = floor(as.numeric(last_predx_visit - birthdt)/365),
    #traditional definition used by ACT
    age_act = ifelse(!is.na(onsetage), onsetage, age_at_last_visit),
    ...
    #ad_dsmivdx = ifelse(dsmivdx == 1, dsmivdx, NA),
     #Time-varying covariates
      #recalculate age_at_exposure to be age at ppl's birthdays. this prevents age_at_exposure_year from being smaller than age_intake at times
      age_at_end_exposure_year = floor(as.numeric(as.Date(paste0(exposure_year, "-", format(birthdt, "%m-%d"))) - birthdt)/365), 
    #"start" of the exposure period should be < "end" of exposure period for time-varying analysis
    age_at_start_exposure_year = age_at_end_exposure_year-1,
    
    ### ? need to change age_at_last_visit to age_act for sensitivity analyses ?
    ## --> EDIT 
    dementia_now = ifelse(age_at_end_exposure_year < age_at_last_visit, 0, anydementia),
    #clean up variables
...
dem <- dem %>%
  ...
  select(
    ...
    age_at_start_exposure_year,
    age_at_end_exposure_year,
    exposure_year:bmi, 
    casi
...
Cases over time. Defining incidence based on date (age) at last study visit. Numbers slightly vary depending on how incidence age is defined. 
...
cases_over_time <- dem %>%
  filter(pollutant=="no2",
         dementia_now==1) %>%
  group_by(study_id) %>%
  #some cases have multiple dementia_now==1 for years After age_last_visit (since this was created wrong & less than age_onset sometimes?)
  filter(exposure_year == min(exposure_year)) %>% 

#check that only 1 study_id per "case" # looks good
#dim(cases_over_time)[1] == length(unique(cases_over_time$study_id))

  group_by(exposure_year) %>%
  summarize(
    cases = sum(dementia_now, na.rm = T)
  ) 

#total number of cases w/ valid No2 exposure 
#sum(cases_over_time$cases)

cases_over_time %>%
  ggplot(aes(x=exposure_year, y=cases)) + 
  geom_point()
...
```{r}
#1. need to calculate age_at_exposure_year at DOB, not Jan 1st
#2. then, cacluate age_prior_to_exposure_year as: age_at_exposure_year - 1. Stop time must be > start time, NA created.

#3. create survival object w/ TVC dataset, accounting for "delayed entry"
# exp: s.stanford.tvc = with(stanford.tvc, Surv(tstart,tstop,death))
dem.no2 <- dem %>%
  filter(pollutant== "no2")

s.dem.no2 <- with(dem.no2, Surv(
  # --> update so that time < time2
  time = age_at_start_exposure_year,  
  time2 =age_at_end_exposure_year,  
  event = dementia_now
                         )
             )
...
dem.no2 %>%
  coxph(s.dem.no2 ~ exp_avg10_yr,# +  cluster(study_id),  #or robust=T ?
        data=., 
        robust = T,
...
```{r}
dem.no2 %>%
  coxph(s.dem.no2 ~ exp_avg10_yr + male + education + factor(race) + exposure_year + strata(apoe), 
        data=., 
        robust = T,
        #ties = "exact"
        ) %>%
...
dem.no2 %>%
  coxph(s.dem.no2 ~ exp_avg10_yr + male + education + factor(race) + exposure_year + strata(apoe) + factor(smoke) + exercise, 
        data=., 
        robust = T,
        #ties = "exact"
        ) %>%
